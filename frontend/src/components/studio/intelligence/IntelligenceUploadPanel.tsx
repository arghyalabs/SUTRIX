...
export const IntelligenceUploadPanel: React.FC<Props> = ({ clientId, apiBase, session, onLoaded }) => {
  ...
  const handleLoadDemo = async () => {
    setLoading(true); setError(null);
    try {
      const r = await fetch(`${apiBase}/api/intelligence/${clientId}/load-demo`, { method: 'POST' });
      const d = await r.json();
      if (!r.ok) throw new Error(d.detail);
      onLoaded(d);
    } catch (e: any) { setError(e.message); }
    ...
    <div className="max-w-4xl mx-auto py-12 flex flex-col items-center w-full">
      <div className="text-center mb-12">
        ...
        <p className="text-secondary text-sm max-w-lg mx-auto">
          Cheminformatics intelligence data ingestion. Upload your dataset to begin the analysis.
        </p>
      </div>
      ...
      <div className="w-full max-w-2xl space-y-6">
        <label
          ...
          className={`relative flex flex-col items-center justify-center w-full h-80 rounded-[2rem] border-2 border-dashed cursor-pointer transition-all duration-300 group overflow-hidden
            ${dragging
              ? 'border-rose-400 bg-rose-400/[0.03] shadow-[0_0_30px_rgba(244,63,94,0.15)]'
        ...
        >
          ...
          <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 transition-transform duration-500
            ${dragging ? 'bg-rose-400 text-void scale-110' : 'bg-white/[0.04] text-secondary group-hover:bg-white/[0.08] group-hover:text-white'}`}>
            {loading ? <Loader2 className="w-8 h-8 text-rose-400 animate-spin" /> : <UploadCloud className="w-8 h-8" />}
          </div>
