...
logger = logging.getLogger("sdo.normalization.formula_parser")

# Standard IUPAC atomic weights (approximate values for general scientific & QSAR use)
ATOMIC_WEIGHTS: Dict[str, float] = {
    ...
    'Rg': 280.0, 'Cn': 285.0, 'Nh': 286.0, 'Fl': 289.0, 'Mc': 288.0, 'Lv': 293.0, 'Ts': 294.0, 'Og': 294.0
}

def parse_formula_weight(formula: str) -> float:
    """
    Parses a chemical formula string (e.g. C8H10N4O2, Pb(NO3)2, (NH4)2SO4)
    and returns its molecular weight. Raises ValueError for malformed formulas.
    """
    ...
    if not formula:
        raise ValueError("Formula cannot be whitespace only.")
    ...
    while i < n:
        ...
        if c == '(':
            ...
            # Extract element name (starts with uppercase, optional lowercase letters)
            start = i
            i += 1
            ...
            if element not in ATOMIC_WEIGHTS:
                raise ValueError(f"Unknown chemical element '{element}' in formula")
            ...
            raise ValueError(f"Unexpected character '{c}' in chemical formula")
