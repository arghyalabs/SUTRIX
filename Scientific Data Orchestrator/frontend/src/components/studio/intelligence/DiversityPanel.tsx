...
const buildHistogram = (values: number[], bins = 20) => {
  ...
  const buckets: { label: string; value: number }[] = Array.from({ length: bins }, (_, i) => ({
...
export const DiversityPanel: React.FC<Props> = ({ clientId, apiBase }) => {
  ...
  const histData: SciBarDatum[] = activeData ? buildHistogram(activeData.histogram?.values ?? []) : [];
    ...
    <div className="space-y-4">
      ...
      {loading && !data && (
        <div className="flex items-center justify-center h-32 gap-2 text-cyan-400">
          <Loader2 className="w-5 h-5 animate-spin" /><span>Analysing diversity…</span>
        </div>
      )}
        ...
        <>
          ...
          {activeData && (
            <>
              ...
              {histData.length > 0 && (
                <SciPanel title={`${activeKey} Distribution`} height={240}>
                  <SciBar
                    data={histData}
                    color="#22D3EE"
          ...
          {availProps.length === 0 && (
            <div className="text-center py-8 text-slate-500 text-sm">
              No physicochemical property columns detected. Add MW, logP, TPSA, HBD, HBA columns.
            </div>
          )}
        </>
      )}
